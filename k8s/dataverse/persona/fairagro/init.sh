...
#  echo
  ...
  echo "Adding @service-account-fairagro_client as admin to dataverse $PARENT_DATAVERSE/$DATAVERSE_ID:"
  curl -X POST -H "Content-Type: application/json" $DATAVERSE_URL/api/dataverses/$DATAVERSE_ID/assignments -d '{"assignee": "@service-account-fairagro_client", "role": "admin"}'
  echo
  ...
  if [[ $PARENT_DATAVERSE == "nfdi4health" ]]; then
    ...
    echo "Adding @service-account-fairagro_client and @dataverseAdmin to curator group"
    curl -X POST -H "Content-Type: application/json" $DATAVERSE_URL/api/dataverses/$DATAVERSE_ID/groups/curators/roleAssignees -d '["@service-account-fairagro_client", "@dataverseAdmin"]'
    echo
