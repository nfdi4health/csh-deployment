...
kubectl cp $(basename $LAST_BACKUP_FILE) $POSTGRES_POD_NAME:/tmp/ --context $DESTINATION_DATAVERSE_CONTEXT -n $DESTINATION_DATAVERSE_NAMESPACE

echo "Unzipping backup..."
kubectl exec $POSTGRES_POD_NAME --context $DESTINATION_DATAVERSE_CONTEXT -n $DESTINATION_DATAVERSE_NAMESPACE -- gunzip /tmp/$(basename $LAST_BACKUP_FILE)
...
kubectl exec $POSTGRES_POD_NAME --context $DESTINATION_DATAVERSE_CONTEXT -n $DESTINATION_DATAVERSE_NAMESPACE -- psql -P pager=off -U dataverse -f /tmp/$(basename $LAST_BACKUP_FILE .gz) template1


echo "Updating database passwords..."
kubectl get secret --context $DESTINATION_DATAVERSE_CONTEXT -n $DESTINATION_DATAVERSE_NAMESPACE | grep ${DESTINATION_DATAVERSE_NAME}-dataverse-postgres.credentials.postgresql.acid.zalan.do | awk '{print $1}' | while read SECRET; do kubectl exec $POSTGRES_POD_NAME --context $DESTINATION_DATAVERSE_CONTEXT -n $DESTINATION_DATAVERSE_NAMESPACE -- psql -P pager=off -U dataverse -c "ALTER USER $(echo $SECRET | awk -F. '{print $1}') WITH PASSWORD '$(kubectl get secrets/$SECRET -o=jsonpath="{.data.password}" --context $DESTINATION_DATAVERSE_CONTEXT -n $DESTINATION_DATAVERSE_NAMESPACE | base64 -d)';"; done
...
need_to_create_solr_backup () {
    ...
    SOLR_BACKUP_RESPONSE=$(kubectl exec -it ${SOURCE_DATAVERSE_NAME}-dataverse-solr-0 --context $SOURCE_DATAVERSE_CONTEXT -n $SOURCE_DATAVERSE_NAMESPACE --container solr -- curl localhost:8983/solr/collection1/replication?command=details)
...
create_solr_backup () {
    ...
    kubectl exec ${SOURCE_DATAVERSE_NAME}-dataverse-solr-0 --context $SOURCE_DATAVERSE_CONTEXT -n $SOURCE_DATAVERSE_NAMESPACE --container solr -- curl -s "localhost:8983/solr/collection1/replication?command=backup&numberToKeep=1"; echo
    while true; do
        SOLR_BACKUP_RESPONSE=$(kubectl exec ${SOURCE_DATAVERSE_NAME}-dataverse-solr-0 --context $SOURCE_DATAVERSE_CONTEXT -n $SOURCE_DATAVERSE_NAMESPACE --container solr -- curl localhost:8983/solr/collection1/replication?command=details)
        SOLR_BACKUP_STATUS=$(echo $SOLR_BACKUP_RESPONSE | jq -r '.details.backup.status')
        if [[ "$SOLR_BACKUP_STATUS" == "success" ]]; then
            break
        fi
...
if kubectl exec ${DESTINATION_DATAVERSE_NAME}-dataverse-solr-0 --container solr --context $DESTINATION_DATAVERSE_CONTEXT -n $DESTINATION_DATAVERSE_NAMESPACE -- sh -c "[ -d /var/solr/data/collection1/data/${SOLR_BACKUP_NAME} ]"; then
...
kubectl exec ${DESTINATION_DATAVERSE_NAME}-dataverse-solr-0 --context $DESTINATION_DATAVERSE_CONTEXT -n $DESTINATION_DATAVERSE_NAMESPACE --container solr -- curl -s "localhost:8983/solr/collection1/replication?command=restore" > /dev/null

while true; do
    SOLR_BACKUP_LOAD_RESPONSE=$(kubectl exec -t ${DESTINATION_DATAVERSE_NAME}-dataverse-solr-0 --context $DESTINATION_DATAVERSE_CONTEXT -n $DESTINATION_DATAVERSE_NAMESPACE --container solr -- curl -s "localhost:8983/solr/collection1/replication?command=restorestatus")
