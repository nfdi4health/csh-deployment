...
kubectl exec -it ${SOURCE_DATAVERSE_NAME}-dataverse-solr-0 --context $SOURCE_DATAVERSE_CONTEXT --container solr -- curl -s "localhost:8983/solr/collection1/replication?command=backup&numberToKeep=1" > /dev/null
while true; do
...
echo "Copying completed backup to destination Solr... (this may take some time)"
SOLR_BACKUP_NAME=$(echo $SOLR_BACKUP_RESPONSE | jq -r '.details.backup.directoryName')
kubectl exec -it ${SOURCE_DATAVERSE_NAME}-dataverse-solr-0 --container solr --context $SOURCE_DATAVERSE_CONTEXT -- tar -zcf /tmp/${SOLR_BACKUP_NAME}.tar.gz /var/solr/data/collection1/data/${SOLR_BACKUP_NAME}
...
kubectl exec -it ${DESTINATION_DATAVERSE_NAME}-dataverse-solr-0 --container solr --context $DESTINATION_DATAVERSE_CONTEXT -- tar -zxf /tmp/${SOLR_BACKUP_NAME}.tar.gz -C /var/solr/data/collection1/data/

kubectl exec -it ${DESTINATION_DATAVERSE_NAME}-dataverse-solr-0 --context $DESTINATION_DATAVERSE_CONTEXT --container solr -- curl -s localhost:8983/solr/collection1/replication?command=restore > /dev/null

while true; do
    SOLR_BACKUP_LOAD_RESPONSE=$(kubectl exec -it ${DESTINATION_DATAVERSE_NAME}-dataverse-solr-0 --context $DESTINATION_DATAVERSE_CONTEXT --container solr -- curl localhost:8983/solr/collection1/replication?command=restorestatus)
    SOLR_BACKUP_LOAD_STATUS=$(echo $SOLR_BACKUP_RESPONSE | jq -r '.restorestatus.status')
